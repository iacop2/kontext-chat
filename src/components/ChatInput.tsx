...
export function ChatInput({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      if ((input.trim() || styleState.selectedStyle) && !isSubmitting && !uploadState.isUploading) {
        onSubmit(e as any);
      }
    }
  };
      ...
      <form onSubmit={onSubmit} className="max-w-3xl mx-auto">
        <div className="relative w-full">
          ...
          <Textarea
            ...
            className={`w-full resize-none rounded border border-stroke-base bg-background px-3 text-sm text-content placeholder:text-content-lighter focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${
              hasAttachments ? 'min-h-[100px] pt-28 pb-10' : 'min-h-[48px] py-2 pb-10'
            }`}
            style={{
              height: 'auto',
              overflow: 'hidden'
          ...
          <div className="absolute bottom-0 right-0 p-2">
            <Button
              ...
              disabled={(!input.trim() && !styleState.selectedStyle) || isSubmitting || uploadState.isUploading}
            >

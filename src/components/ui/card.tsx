...
>(({ className, compact, ...props }, ref) => (
	<div
		...
		className={cn(
			'border-stroke-light bg-surface flex flex-col rounded border',
			'group',
			className
		)}
		{...props}
...
>(({ className, ...props }, ref) => (
	<h3
		ref={ref}
		className={cn(
			'font-base text-content-base font-heading text-lg leading-none tracking-tight lg:text-xl',
			className
		)}
		{...props}
	/>
...
>(({ className, ...props }, ref) => (
	<p
		ref={ref}
		className={cn('text-content-light text-sm', className)}
		{...props}
	/>
));

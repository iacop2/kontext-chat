...
function processMessagesWithFiles(messages: UIMessage[]): UIMessage[] {
	return messages.map((msg) => {
		if (msg.role === 'user' && msg.parts) {
			...
			const loraParts = msg.parts.filter(
				(part) => part.type === 'data-lora-selection'
			);
			...
			const context = {
				userPrompt,
				imageAttached:
					fileParts.length > 0 ? fileParts.map((part) => part.url) : null,
				loraSelected:
					loraParts.length > 0
						? loraParts.map((part) => {
...
async function processImageGeneration(
	...
	try {
		...
		if (TEST_MODE) {
			return await processTestModeGeneration(
				writer,
				toolCallId,
				prompt,
				type,
				...
				try {
					const result = streamText({
						...
						tools: {
							createImage: tool({
								...
								inputSchema: z.object({
									...
									loraUrl: z
										...
										.url()
										.optional()
										.describe('Optional LoRA URL for style application'),
								}),
							...
							editImage: tool({
								...
								inputSchema: z.object({
									...
									imageUrl: z
										.string()
										.url()
										.describe('The URL of the image to edit'),
									loraUrl: z
										.string()
										.url()
										.optional()
							...
							describeImage: tool({
								description:
									'Gets a detailed text description of an image from a URL.',
								inputSchema: z.object({
									imageUrl: z
										.string()
										.url()
										.describe('The URL of the image to describe.'),
								}),
